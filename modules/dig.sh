...
bash -c "dig --short A ${DOMAIN} +noall +answer | tee ${OUTFILE}"
printf "${BLUE}[+] Starting CNA<E record scan${NC}"
bash -c "dig --short CNAME ${DOMAIN} +noall +answer | tee ${OUTFILE}"
printf "${BLUE}[+] Starting TXT record scan${NC}"
bash -c "dig --short TXT ${DOMAIN} +noall +answer | tee ${OUTFILE}"
printf "${BLUE}[+] Starting MX record scan${NC}"
bash -c "dig --short MX ${DOMAIN} +noall +answer | tee ${OUTFILE}"
